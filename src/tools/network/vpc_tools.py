        ...
        for rule in sg.get("IpPermissions", []):
            inbound_rules.append({
                ...
                "sources": [
                    ...
                    sg_pair.get("GroupId") for sg_pair in rule.get("UserIdGroupPairs", [])
                ],
            })
...
def describe_network_acls(
    ...
    Args:
        network_acl_ids: 조회할 네트워크 ACL ID 목록 (선택)
        vpc_id: 특정 VPC의 네트워크 ACL만 조회 (선택)
    ...
    for nacl in response.get("NetworkAcls", []):
        name_tag = next(
            (tag["Value"] for tag in nacl.get("Tags", []) if tag["Key"] == "Name"),
            "N/A",
        )
        ...
        network_acls.append({
            ...
            "inbound_rules": sorted(inbound_rules, key=lambda x: x["rule_number"]),
            "outbound_rules": sorted(outbound_rules, key=lambda x: x["rule_number"]),
            "associated_subnets": [
...
def analyze_network_topology(vpc_id: str) -> dict[str, Any]:
    ...
    network_acls = describe_network_acls(vpc_id=vpc_id)

    # 서브넷을 Public/Private로 분류
    public_subnets = []
    private_subnets = []
    ...
    return {
        ...
        "summary": {
            ...
            "network_acls": network_acls.get("total_count", 0),
        ...
        "network_acls": network_acls.get("network_acls", []),
    }
